...
function AnimatedAreaChart({ data, dataKeys, colors, width = 700, height = 300, delay = 0 }) {
  ...
  const allValues = data.flatMap((d) => dataKeys.map((k) => d[k])); const maxVal = Math.max(...allValues); const range = maxVal || 1; const xStep = chartW / (data.length - 1);
  const gridLines = 5; const gridVals = Array.from({ length: gridLines }, (_, i) => Math.round((range / (gridLines - 1)) * i));
  const handleMouseMove = useCallback((e) => { if (!ref.current) return; const rect = ref.current.getBoundingClientRect(); const x = e.clientX - rect.left - padding.left; const idx = Math.round(x / xStep); setActivePoint(idx >= 0 && idx < data.length ? idx : null); }, [data.length, xStep]);
  ...
  return (
    <div style={{ width: "100%", maxWidth: width, overflow: "visible" }}>
      <svg ref={ref} width="100%" viewBox={`0 0 ${width} ${height}`} style={{ overflow: "visible" }} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
        ...
        {data.map((d, i) => (<text key={i} x={padding.left + i * xStep} y={height - 8} textAnchor="middle" fill={C.mutedLight} fontSize={11}>{d.month || d.day}</text>))}
        ...
        <g clipPath={`url(#chart-clip-${delay})`}>
          {dataKeys.map((key, si) => {
            const pts = data.map((d, i) => ({ x: padding.left + i * xStep, y: padding.top + chartH - (d[key] / range) * chartH }));
      ...
      {activePoint !== null && (
        <div style={{ position: "absolute", left: `${((padding.left + activePoint * xStep) / width) * 100}%`, top: 8, transform: "translateX(-50%)", background: C.ink, borderRadius: 10, padding: "10px 14px", boxShadow: "0 8px 24px rgba(15,51,82,0.25)", zIndex: 10, pointerEvents: "none", whiteSpace: "nowrap" }}>
          <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 12, color: C.white, marginBottom: 6 }}>{data[activePoint].month || data[activePoint].day}</div>
          {dataKeys.map((key, i) => (<div key={key} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: C.navy100 }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: colors[i], flexShrink: 0 }} /><span style={{ color: C.white, fontWeight: 600 }}>{formatNumber(data[activePoint][key])}</span></div>))}
        </div>
      )}
    </div>
  );
