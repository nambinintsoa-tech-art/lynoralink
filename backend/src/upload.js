...
export async function registerUploadRoutes(app) {
  ...
  app.post("/v1/upload", async (request, reply) => {
    ...
    if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) return reply.code(503).send({ error: "Configuration Cloudinary backend manquante" });
